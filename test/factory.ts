...
describe("Factory Contract Test", function () {
  ...
  it("should deploy Factory contract", async function () {
    const SilverFactoryInstance = await ethers.getContractFactory("SilverPadFactory");
    silverPadFactory = await SilverFactoryInstance.deploy(await dai.getAddress(), daoAddress);
  ...
  it("test for deploying Silver ICO", async function () {
    ...
    console.log("user's fee before: ", await silverPadFactory.feeContributions(await user.address));
    console.log("user's dai balance before: ", await dai.balanceOf(await user.address));
    console.log("user's isPaidSpmaFilterFee", await silverPadFactory.paidSpamFilterFee(user.address));

    await dai.connect(user).approve(await silverPadFactory.getAddress(), ethers.parseEther("100"));
    await silverPadFactory.connect(user).paySpamFilterFee();

    console.log("user's fee: ", await silverPadFactory.feeContributions(await user.address));
    console.log("user's dai balance: ", await dai.balanceOf(await user.address));
    console.log("user's isPaidSpmaFilterFee", await silverPadFactory.paidSpamFilterFee(user.address));
    ...
    await silverPadFactory.connect(user).launchNewICO(
      _projectURI,
      _softcap,
      _hardcap,
      _endTime,
    ...
    // console.log("user's fee: ", await silverPadFactory.feeContributions(await user.address))
    const _silvers = await silverPadFactory.getVulcans();
    console.log("ICOs: ", _silvers);

    const silverInstance = await ethers.getContractFactory("Silver");
    ico = await silverInstance.attach(_silvers[0]);
  });
  ...
  it("Check the balance of all addresses", async function () {
    ...
    console.log("ico tokens left", ethers.formatUnits(tokensRemain, 10));
    console.log("user: ", {
